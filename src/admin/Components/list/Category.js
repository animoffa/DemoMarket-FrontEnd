...
let Category = (props) => {
    return (
        <div className={CategoryCss.categoryMain}>
            ...
            <div>{props.ProductsOfOrder ? <div className={CategoryCss.count}>{props.ProductsOfOrder}</div> : <NavLink  to={props.categoryName ? `/changecategory/${props.id}` : `/changeproduct/${props.id}` }><img src={edit} alt={edit}/></NavLink>}
            </div>
            <div>{props.StausOfOrder ? <div>{props.StausOfOrder}</div> : <img src={del} alt={del} onClick={props.delete}/>}
            </div>
        </div>
    )
};
